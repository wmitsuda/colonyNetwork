...
import { UINT256_MAX, MAX_INT128, WAD, MIN_STAKE, DEFAULT_STAKE, MINING_CYCLE_DURATION, DECAY_RATE, ZERO_ADDRESS } from "../helpers/constants";
...
contract("ColonyNetworkMining", accounts => {
  ...
  describe("Submission eligibility", () => {
    it("should not allow someone to submit a new reputation hash if they are ineligible", async () => {
      ...
      await checkErrorRevert(
        repCycle.submitRootHash("0x12345678", 10, 10, { from: MAIN_ACCOUNT }),
    ...
    it("should not allow someone to submit a new reputation hash to the next ReputationMiningCycle", async () => {
      ...
      await checkErrorRevert(repCycle.submitRootHash("0x12345678", 10, 10, { from: MAIN_ACCOUNT }), "colony-reputation-mining-cycle-not-open");
      const nSubmittedHashes = await repCycle.getNSubmittedHashes();
    ...
    it("should allow someone to submit a new reputation hash if they are eligible inside the window", async () => {
      ...
      await repCycle.submitRootHash("0x12345678", 10, entryNumber, { from: MAIN_ACCOUNT });
      await forwardTime(MINING_CYCLE_DURATION / 2, this);
    });
    ...
    it("should not allow a user to back more than one hash in a single cycle", async () => {
      ...
      await checkErrorRevert(
        repCycle.submitRootHash("0x87654321", 10, entryNumber2, { from: MAIN_ACCOUNT }),
    ...
    it("should not allow a user to back the same hash with different number of nodes in a single cycle", async () => {
      ...
      await checkErrorRevert(
        repCycle.submitRootHash("0x12345678", 11, entryNumber, { from: MAIN_ACCOUNT }),
    ...
    it("should not allow a user to submit the same entry for the same hash twice in a single cycle", async () => {
      ...
      await checkErrorRevert(
        repCycle.submitRootHash("0x12345678", 10, entryNumber, { from: MAIN_ACCOUNT }),
    ...
    it("should allow a user to back the same hash more than once in a same cycle with different entries, and be rewarded", async () => {
      ...
      await repCycle.submitRootHash("0x12345678", 10, entryNumber, { from: MAIN_ACCOUNT });
      await repCycle.submitRootHash("0x12345678", 10, entryNumber2, { from: MAIN_ACCOUNT });
      const nSubmittedHashes = await repCycle.getNSubmittedHashes();
      assert.isTrue(nSubmittedHashes.eq(new BN(1)));
      await forwardTime(MINING_CYCLE_DURATION / 2, this);
    ...
    it("should only allow 12 entries to back a single hash in each cycle", async () => {
      ...
      for (let i = 1; i <= 12; i += 1) {
        await repCycle.submitRootHash("0x12345678", 10, entryNumber, { from: MAIN_ACCOUNT }); // eslint-disable-line no-await-in-loop
    ...
    it("should prevent submission of hashes with an invalid entry for the balance of a user", async () => {
      ...
      await forwardTime(MINING_CYCLE_DURATION, this);
      await checkErrorRevert(
        repCycle.submitRootHash("0x12345678", 10, 1000000000000, { from: MAIN_ACCOUNT }),
    ...
    it("should prevent submission of hashes with a valid entry, but invalid hash for the current time", async () => {
      ...
      const repCycle = await getActiveRepCycle(colonyNetwork);
      await checkErrorRevert(
        repCycle.submitRootHash("0x12345678", 10, 1, { from: MAIN_ACCOUNT }),
        "colony-reputation-mining-cycle-submission-not-within-target"
      );
    });
  });
  ...
  describe("Types of disagreement", () => {
    ...
    it.skip("if a new reputation's uniqueID is wrong, that disagreement should be handled correctly", async () => {
      ...
      // update cycle, and reputation updates for three task completions (manager, worker (skill and domain), evaluator);
      // That's thirteen in total.
      // TODO: rename nInactiveLogEntries to activeLogEntries as this is what's happening here
      // It must have been missed when we switched the active/inactive meaning for the rep cycles
      const nInactiveLogEntries = await repCycle.getReputationUpdateLogLength();
      assert.equal(nInactiveLogEntries.toNumber(), 13);
    ...
    it("if update makes reputation amount go over the max, in a dispute, it should be limited to the max value", async () => {
      await giveUserCLNYTokensAndStake(colonyNetwork, MAIN_ACCOUNT, DEFAULT_STAKE);
      await giveUserCLNYTokensAndStake(colonyNetwork, OTHER_ACCOUNT, DEFAULT_STAKE);
      const fundsRequired = MAX_INT128.add(toBN(1000000000000)).add(toBN(1000000000));
  ...
  describe("Misbehaviour during dispute resolution", () => {
    ...
    it("should correctly check the UID of the reputation if the reputation update being disputed is a decay", async () => {
      ...
      const agreeStateReputationUIDFake = new BN(
        goodClient.justificationHashes[`0x${new BN(firstDisagreeIdx).toString(16, 64)}`].justUpdatedProof.uid.slice(2),
        16
      )
        .addn(1)
        .toString(16, 64);
    ...
    it("should not allow stages to be skipped even if the number of updates is a power of 2", async () => {
      ...
      while (!powerTwoEntries) {
        ...
        // console.log("lastLogEntry", lastLogEntry.toString());
  ...
  describe("Intended ('happy path') behaviours", () => {
    ...
    it("should cope if someone's reputation would overflow, setting it to the maximum value instead", async () => {
      ...
      badClient = new MaliciousReputationMinerExtraRep(
        { loader: contractLoader, minerAddress: OTHER_ACCOUNT, realProviderPort: REAL_PROVIDER_PORT, useJsTree },
        29,
        new BN("-10").pow(new BN("34")).muln(2)
      );
      await badClient.initialise(colonyNetwork.address);
      ...
      await goodClient.insert(globalKey, new BN("2").pow(new BN("127")).subn(2), 0);
      await goodClient.insert(userKey, new BN("2").pow(new BN("127")).subn(2), 0);
      await badClient.insert(globalKey, new BN("2").pow(new BN("127")).subn(2), 0);
      await badClient.insert(userKey, new BN("2").pow(new BN("127")).subn(2), 0);

      const rootHash = await goodClient.getRootHash();
      await fundColonyWithTokens(metaColony, clny, new BN("4").mul(new BN("10").pow(new BN("34"))).toString());
      await setupFinalizedTask({
        colonyNetwork,
        colony: metaColony,
        worker: MAIN_ACCOUNT,
        managerPayout: new BN("10").pow(new BN("34")).toString(),
        evaluatorPayout: new BN("10").pow(new BN("34")).toString(),
        workerPayout: new BN("10").pow(new BN("34")).toString(),
        managerRating: 3,
        workerRating: 3
      });
    ...
    it("should insert reputation updates from the log", async () => {
      ...
      assert.equal(Object.keys(client.reputations).length, 27);
      ...
      const reputationProps = [
        ...
        // The subtracted amounts below are decays
        {
          id: 1,
          skill: META_ROOT_SKILL,
          account: undefined,
          value: DEFAULT_STAKE.muln(6)
            .add(REWARD)
            .sub(new BN(5999000000000))
        },
        { id: 2, skill: MINING_SKILL, account: undefined, value: REWARD },
        {
          ...
          value: DEFAULT_STAKE.muln(6).add(REWARD)
        },
        { id: 4, skill: MINING_SKILL, account: MAIN_ACCOUNT, value: REWARD },
        // giveUserCLNYTokens * 3
        { id: 5, skill: META_ROOT_SKILL, account: OTHER_ACCOUNT2, value: "0" },
        { id: 6, skill: GLOBAL_SKILL, account: undefined, value: "0" },
        { id: 7, skill: GLOBAL_SKILL, account: OTHER_ACCOUNT2, value: "0" },
        // Completing a task in skill 4
        { id: 8, skill: MINING_SKILL, account: accounts[0], value: "0" },
        { id: 9, skill: META_ROOT_SKILL, account: accounts[0], value: "0" },
        { id: 10, skill: META_ROOT_SKILL, account: accounts[1], value: "1000000000" },
        { id: 11, skill: MINING_SKILL, account: accounts[3], value: "0" },
        { id: 12, skill: META_ROOT_SKILL, account: accounts[3], value: "0" },
        { id: 13, skill: new BN(5), account: undefined, value: "0" },
        { id: 14, skill: new BN(6), account: undefined, value: "0" },
        { id: 15, skill: new BN(7), account: undefined, value: "0" },
        { id: 16, skill: new BN(8), account: undefined, value: "0" },
        { id: 17, skill: new BN(9), account: undefined, value: "0" },
        { id: 18, skill: new BN(10), account: undefined, value: "0" },
        { id: 19, skill: new BN(4), account: undefined, value: "0" },
        { id: 20, skill: new BN(5), account: accounts[3], value: "0" },
        { id: 21, skill: new BN(6), account: accounts[3], value: "0" },
        { id: 22, skill: new BN(7), account: accounts[3], value: "0" },
        { id: 23, skill: new BN(8), account: accounts[3], value: "0" },
        { id: 24, skill: new BN(9), account: accounts[3], value: "0" },
        { id: 25, skill: new BN(10), account: accounts[3], value: "0" },
        { id: 26, skill: GLOBAL_SKILL, account: accounts[3], value: "0" },
        { id: 27, skill: new BN(4), account: accounts[3], value: "0" }
      ];
    ...
    it("should correctly update child reputations", async () => {
      ...
      await client.initialise(colonyNetwork.address);
      await client.addLogContentsToReputationTree();
      assert.equal(Object.keys(client.reputations).length, 26);
      const GLOBAL_SKILL = new BN(1);
      const META_ROOT_SKILL = new BN(2);
      const MINING_SKILL = new BN(3);

      const reputationProps = [
        { id: 1, skill: META_ROOT_SKILL, account: undefined, value: "4001200000005802000000000" },
        { id: 2, skill: MINING_SKILL, account: undefined, value: REWARD },
        { id: 3, skill: META_ROOT_SKILL, account: MAIN_ACCOUNT, value: "4001200000000000000000000" },
        { id: 4, skill: MINING_SKILL, account: MAIN_ACCOUNT, value: REWARD },
        { id: 5, skill: META_ROOT_SKILL, account: OTHER_ACCOUNT2, value: 0 },
        { id: 6, skill: GLOBAL_SKILL, account: undefined, value: 3300000000000 },
        { id: 7, skill: GLOBAL_SKILL, account: OTHER_ACCOUNT2, value: 0 },
        { id: 8, skill: META_ROOT_SKILL, account: accounts[0], value: 2500000000000 },
        { id: 9, skill: META_ROOT_SKILL, account: accounts[1], value: 2000000000 },
        { id: 10, skill: META_ROOT_SKILL, account: accounts[2], value: 3300000000000 },
        { id: 11, skill: new BN(9), account: undefined, value: 3300000000000 },
        { id: 12, skill: new BN(8), account: undefined, value: 3300000000000 },
        { id: 13, skill: new BN(7), account: undefined, value: 3300000000000 },
        { id: 14, skill: new BN(6), account: undefined, value: 3300000000000 },
        { id: 15, skill: new BN(5), account: undefined, value: 3300000000000 },
        { id: 16, skill: new BN(4), account: undefined, value: 3300000000000 },
        { id: 17, skill: new BN(10), account: undefined, value: 3300000000000 },
        { id: 18, skill: new BN(9), account: accounts[2], value: 3300000000000 },
        { id: 19, skill: new BN(8), account: accounts[2], value: 3300000000000 }, // 44% decrease
        { id: 20, skill: new BN(7), account: accounts[2], value: 3300000000000 },
        { id: 21, skill: new BN(6), account: accounts[2], value: 3300000000000 },
        { id: 22, skill: new BN(5), account: accounts[2], value: 3300000000000 },
        { id: 23, skill: new BN(4), account: accounts[2], value: 3300000000000 },
        { id: 24, skill: new BN(1), account: accounts[2], value: 3300000000000 },
        { id: 25, skill: new BN(10), account: accounts[2], value: 3300000000000 },
        { id: 26, skill: MINING_SKILL, account: accounts[2], value: 0 }
      ];
